...
class Parser {
    ...
    private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
        int height, width, xOffset, sides, radius;
        ...
        verifyNextToken(Token.TOPVERTEX);
        int[] topVertex = getNumberList(2);
        Point pointTopVertex = new Point(topVertex[0], topVertex[1]);
        verifyNextToken(Token.UPPERLEFT);
        int[] upperLeft = getNumberList(2);
        Point pointUpperleft = new Point(upperLeft[0], upperLeft[1]);
        verifyNextToken(Token.LOWERRIGHT);
        int[] lowerRight = getNumberList(2);
        Point pointLowerRight = new Point(lowerRight[0], lowerRight[1]);
        verifyNextToken(Token.CENTER);
        int[] center = getNumberList(2);
        Point pointCenter = new Point(center[0], center[1]);
        if (imageToken == Token.RightTriangle) {
            ...
            RightTriangle rtriangle = new RightTriangle(color, pointUpperleft, height, width);
            scene.addImage(rtriangle);
        } else if (imageToken == Token.IsoscelesTriangle) {
            verifyNextToken(Token.HEIGHT);
            verifyNextToken(Token.NUMBER);
            height = lexer.getNumber();
            verifyNextToken(Token.WIDTH);
            verifyNextToken(Token.NUMBER);
            width = lexer.getNumber();
            IsoscelesTriangle itriangle = new IsoscelesTriangle(color, pointTopVertex, height, width);
            scene.addImage(itriangle);
        } else if (imageToken == Token.Parallelogram) {
            verifyNextToken(Token.XOFFSET);
            verifyNextToken(Token.NUMBER);
            xOffset = lexer.getNumber();
            Parallelogram parallelogram = new Parallelogram(color, pointUpperleft, pointLowerRight, xOffset);
            ...
            RegularPolygon regularpolygon = new RegularPolygon(color, pointCenter, sides, radius);
